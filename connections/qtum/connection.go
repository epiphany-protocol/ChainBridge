...
func NewConnection(endpoint string, http bool, fromAddress string, log log15.Logger, gasLimit, maxGasPrice, minGasPrice *big.Int, gasMultiplier *big.Float, gsnApiKey, gsnSpeed string) *Connection {
	return &Connection{
		endpoint:      endpoint,
		http:          http,
...
func (c *Connection) Send(method string, data []byte) error {
	return c.rpcClient.CallContext(ensureContext(c.opts.Context), nil, method, hexutil.Encode(data))
}
